...
Добавлена возможность использовать SAST Semgrep. В CI\CD он не интегрирован. Для того, чтобы проанализировать проект на наличие уязвимостей необходимо выполнить `docker run --rm -v .:/src returntocorp/semgrep semgrep --config auto`. Данная команда выведет всевозможные уязвимости, которые найдет.

Добавлены некоторые свои правила для Semgrep, они в файле `.semgrep.yml`.  В CI\CD он не интегрирован. Для того, чтобы проанализировать проект на наличие уязвимостей необходимо выполнить `docker run --rm -v .:/src returntocorp/semgrep semgrep --config .semgrep.yml`. Данная команда выведет уязвимости, которые указывались в правилах в указанном файле, которые найдет.
...
* [X] Улучшен функционал. Добавлены тесты, а так же интегрированы в сборку проекта. Написаны правила semgrep 
